...
def affineLKtracker(T, img, rect, p_prev):
    ...
    for i in range(1000):
        ...
        I = gammaCorrection(I, 1.5)  # Correcting image
        ...
        p_prev += 30 * dp  # update change in p_prev
        p_prev = p_prev.reshape(6, )  # convert p_prev back to array

        if (np.linalg.norm(dp) <= 0.089):
            return p_prev
    return p_prev
...
def main():
    ...
    out = cv2.VideoWriter('trackboltrobust.avi', fourcc, 5.0, (images[0].shape[1], images[0].shape[0]))
